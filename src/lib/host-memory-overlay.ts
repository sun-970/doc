...
import { askLayaDocumentRanking, type LayaAsk } from '@/lib/laya-client'
...
export type HostMemoryAsk = LayaAsk
...
export async function requestHostMemoryAdvice(
  ...
  if (!hostMemoryLayaEnabled(env)) return { ranking, called: false }
  const payload: Record<string, unknown> = { ids: ranking.candidates }
  assertMetadataOnlyAdvicePayload(payload)
  const ask = deps.ask ?? ((body) => askLayaDocumentRanking(body, { env }))
  const overlayOrder = await ask(payload)
  if (!overlayOrder || overlayOrder.some((id) => !ranking.candidates.includes(id))) {
